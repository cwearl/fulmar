...
(define/contract (string-type? g)
  ...
  (or (string? g)
      (symbol? g)))
(provide string-type?)
(define/contract (literal-type? g)
  (-> any/c boolean?)
  (or (indent? g)
      (string-type? g)))
(provide literal-type?)
(define/contract (literal-list? lst)
  (-> any/c boolean?)
  (or (literal-type? lst)
      (and (list? lst)
           (andmap literal-list? lst))))
(provide literal-list?)
...
(define/contract (chunk? c)
  ...
  (or (chunk-literal? c)
      (chunk-struct? c)))
(provide chunk?)
;TODO: Determine if chunk-list? and nullable-chunk-list? are needed
