...
(define/provide-test-suite test-build-indentation
  (test-case
   ...
   (check-equal? (build-indentation (context 0 80 (empty-env))) "")
   (check-equal? (build-indentation (context 4 80 (empty-env))) "    ")
   (check-equal? (build-indentation (context 4 2 (empty-env))) "    "))
  (test-case
...
(define/provide-test-suite test-finish-line
  (test-case
   ...
   (define test-context (context 0 6 (empty-env)))
...
(define (check-speculative-line-length-old cnt line context)
    (check-speculative-line-length (make-whitespace cnt) line context))

(define/provide-test-suite test-check-speculative-line-length  
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - empty string"
   (define test-context (context 0 6 (empty-env)))
   (check-true (check-speculative-line-length-old 0 "" test-context))
   (check-true (check-speculative-line-length-old 1 "" test-context))
   (check-true (check-speculative-line-length-old 3 "" test-context))
   (check-true (check-speculative-line-length-old 5 "" test-context))
   (check-true (check-speculative-line-length-old 6 "" test-context))
   (check-false (check-speculative-line-length-old 7 "" test-context))
   (check-false (check-speculative-line-length-old 10 "" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - single-element string"
   (define test-context (context 0 6 (empty-env)))
   (check-true (check-speculative-line-length-old 0 "1" test-context))
   (check-true (check-speculative-line-length-old 1 "1" test-context))
   (check-true (check-speculative-line-length-old 3 "1" test-context))
   (check-true (check-speculative-line-length-old 5 "1" test-context))
   (check-false (check-speculative-line-length-old 6 "1" test-context))
   (check-false (check-speculative-line-length-old 7 "1" test-context))
   (check-false (check-speculative-line-length-old 10 "1" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - half string"
   (define test-context (context 0 6 (empty-env)))
   (check-true (check-speculative-line-length-old 0 "123" test-context))
   (check-true (check-speculative-line-length-old 1 "123" test-context))
   (check-true (check-speculative-line-length-old 3 "123" test-context))
   (check-false (check-speculative-line-length-old 5 "123" test-context))
   (check-false (check-speculative-line-length-old 6 "123" test-context))
   (check-false (check-speculative-line-length-old 7 "123" test-context))
   (check-false (check-speculative-line-length-old 10 "123" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - long string"
   (define test-context (context 0 6 (empty-env)))
   (check-true (check-speculative-line-length-old 0 "12345" test-context))
   (check-true (check-speculative-line-length-old 1 "12345" test-context))
   (check-false (check-speculative-line-length-old 3 "12345" test-context))
   (check-false (check-speculative-line-length-old 5 "12345" test-context))
   (check-false (check-speculative-line-length-old 6 "12345" test-context))
   (check-false (check-speculative-line-length-old 7 "12345" test-context))
   (check-false (check-speculative-line-length-old 10 "12345" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - max-length string"
   (define test-context (context 0 6 (empty-env)))
   (check-true (check-speculative-line-length-old 0 "123456" test-context))
   (check-false (check-speculative-line-length-old 1 "123456" test-context))
   (check-false (check-speculative-line-length-old 3 "123456" test-context))
   (check-false (check-speculative-line-length-old 5 "123456" test-context))
   (check-false (check-speculative-line-length-old 6 "123456" test-context))
   (check-false (check-speculative-line-length-old 7 "123456" test-context))
   (check-false (check-speculative-line-length-old 10 "123456" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - slightly too long string"
   (define test-context (context 0 6 (empty-env)))
   (check-false (check-speculative-line-length-old 0 "1234567" test-context))
   (check-false (check-speculative-line-length-old 1 "1234567" test-context))
   (check-false (check-speculative-line-length-old 3 "1234567" test-context))
   (check-false (check-speculative-line-length-old 5 "1234567" test-context))
   (check-false (check-speculative-line-length-old 6 "1234567" test-context))
   (check-false (check-speculative-line-length-old 7 "1234567" test-context))
   (check-false (check-speculative-line-length-old 10 "1234567" test-context)))
  (test-case
   "Test check-speculative-line-length-old (check against context's line length - too long string"
   (define test-context (context 0 6 (empty-env)))
   (check-false (check-speculative-line-length-old 0 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 1 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 3 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 5 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 6 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 7 "1234567890" test-context))
   (check-false (check-speculative-line-length-old 10 "1234567890" test-context))))
...
(define/provide-test-suite test-add-literal
  ...
  (test-case
   ...
   (define test-context (context 3 6 (empty-env)))
   (check-equal? (add-literal 'normal "" test-context "") '(""))
   (check-equal? (add-literal 'normal "abc" test-context "") '("abc"))
   (check-equal? (add-literal 'normal "abcd" test-context "") '("abcd"))
   (check-equal? (add-literal 'normal "abcdefg" test-context "") '("abcdefg")))
  ...
  (test-case
   ...
   (define test-context (context 6 6 (empty-env)))
   (check-equal? (add-literal 'normal "" test-context "123") '("123"))
   (check-equal? (add-literal 'normal "abc" test-context "123") '("123abc"))
   (check-equal? (add-literal 'normal "abcd" test-context "123") '("123abcd"))
   (check-equal? (add-literal 'normal "abcdefg" test-context "123") '("123abcdefg")))
...
(define/provide-test-suite test-add-empty
  (test-case
   ...
   (define test-context (context 0 6 (empty-env)))
   (define test-context-1 (context 1 6 (empty-env)))
...
(define/provide-test-suite test-add-speculative
  (test-case
   ...
   (define test-context (context 0 20 (empty-env)))
